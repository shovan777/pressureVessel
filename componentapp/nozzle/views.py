...
class NozzleAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            row_dict_nozzle = NozzleData.objects.filter(class_value=data1.get('class_value')).filter(
                type_name=data1.get('type_name')).filter(nominal_pipe_size=data1.get('nominal_pipe_size')).values()[0]
        ...
        shellAllowableStress = max_stress_calculator(
        ...
        kwargs = {
            ...
            'shellAllowableStress': shellAllowableStress*1000,
            'yieldStrength': yieldStrength*1000,
            'cylinderInsideDiameter': cylinderInsideDiameter,
            'cylinderThickness': cylinderThickness,
            'nozzleOutsideDiameter': nozzleOutsideDiameter,
            'nozzleThickness': nozzleThickness,
            'externalNozzleProjection': externalNozzleProjection,
            'internalNozzleProjection': internalNozzleProjection,
            'nozzleAllowableStress': nozzleAllowableStress,
            'reinforcingElementAllowableStress': reinforcingElementAllowableStress,
            'report_id': projectID,
            'component_react_id': component_react_id,
        }

        # value = calculation_thick(designPressure, corrosionAllowance, shellAllowableStress*1000,
        ...
        value = calculation_thick(**kwargs)

        newdict1 = row_dict_nozzle

        # TODO: calculate weight for the nozzle
