...
class VesselBuilder extends Component {
  ...
  getRequest = () => {
    axios.get("/api/cylinder/" + 506 + "/").then(response => {
  ...
  postRequest = () => {
    axios.post("/api/data/", JSON.stringify({name: 'hero'})).then(response => {
      console.log(response);
    });
  }
  ...
  render() {
    ...
    return (<Aux>
      <Modal show={this.state.showParam && this.state.showEllipsoid}>
        ...
        <Parameter2 label={"Ellipsoidal Head Dimensions"} show={!this.state.showParam1} submitParams={this.submitParamsHandler2} cancelParams={this.cancelParamsHandler} previousParams={this.previousParamsHandler} min1={0.3625} min2={0.3625}/>
      </Modal>
      ...
      <Modal show={this.state.showParam && this.state.showCylinder}>
        ...
        <Parameter3 label={"Cylinder Dimensions"} show={!this.state.showParam3} submitParams={this.submitParamsHandler3} cancelParams={this.cancelParamsHandler} previousParams={this.previousParamsHandler} min={0.3625}/>
      </Modal>
